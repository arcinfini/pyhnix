...
class Phoenix(commands.Bot):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.guild = discord.Object(id=config["server"])
    ...
    async def __awake_hook(self):
        ...
        logger.info("Awake as @%s#%s", user.name, user.discriminator)

    async def setup_hook(self):
        """
        Sets up important extensions for the bot after login
        """
        ...
        for item in extdir.iterdir():
            ...
            if item.suffix == ".py":
                ...
                try:
                    ...
                    logger.error(
                        "an error occurred while loading extension", exc_info=e
                    )
    ...
    async def ensure_database(self):
        ...
        if self.__pool is not None and not self.__pool._closed:
            return
