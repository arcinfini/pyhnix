...
class EmbedBuilderView(dui.View):
    ...
    def update_field_select(self):
        ...
        if len(options) == 0:
            options.append(discord.SelectOption(label="Null"))
    ...
    async def prompt(
        self, interaction: Interaction, button: dui.Button, **kwargs
    ):
        """
    ...
    async def on_error(
        self, interaction: Interaction, error, element: dui.Item
    ):
        """
    ...
    async def _author(self, interaction: Interaction, button: dui.Button):
        ...
        self.embed.set_author(
            name=response.name, url=response.url, icon_url=response.icon_url
        )
    ...
    async def _footer(self, interaction: Interaction, button: dui.Button):
        ...
        self.previous_embed = self.__copy_embed()
        self.embed.set_footer(text=response.text, icon_url=response.icon_url)
    ...
    async def _create_field(self, interaction: Interaction, button: dui.Button):
        ...
        response = await self.prompt(
            ...
            inline=self.input(
                "inline: 0 for false and 1 for true",
                default="0",
                required=True,
                style=discord.TextStyle.short,
                max_length=1,
    ...
    async def _import(self, interaction: Interaction, button: dui.Button):
        ...
        response = await self.prompt(
            ...
            button,
            content=self.input(
                "embed json", required=True, style=discord.TextStyle.long
            ),
        )
    ...
    async def _export(self, interaction: Interaction, button: dui.Button):
        ...
        string = io.BytesIO(
            bytes(json.dumps(self.embed.to_dict(), indent=4), encoding="utf-8")
        )
...
class Main(app_commands.Group):
    ...
    async def _build(self, interaction: Interaction):
        """
    ...
    async def edit(interaction: Interaction, message: discord.Message):
        if message.author.id != interaction.client.user.id:
            raise app_commands.CheckFailure(
                "I do not have permission to edit this message"
            )
        ...
        await interaction.response.send_message(
            embed=embed, view=builder, ephemeral=True
        )
